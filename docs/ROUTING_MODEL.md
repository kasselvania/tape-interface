...
isolation. The operator then confirmed both meters drop instantly with BTN3 ZERO,
associating the residual with tone activation rather than a steady noise floor.
Its frequency and coupling path remain unmeasured. Mic Input pickup was
suggested by the operator but is unproven.
...
| P3 | Held TONE OUT L then ZERO; record USB Capture L/R and each physical sink | Speaker/USB Capture signal observed. Residual other-channel activity corrects initial right-flat report; both meters drop instantly on ZERO. Isolation UNRESOLVED; Output Jack untested. |
| P4 | Held TONE OUT R then ZERO; same measurements | Operator confirms right mode tried after correcting prior left-mode mistake; residual reported with both tones. Subsequent ZERO drops both meters. Isolation and meter identity UNRESOLVED; no measured capture. |
