...
Speaker and USB Capture left with Monitor Flag OFF. After correcting a mode
mix-up, the operator initially reported activity on the other channel, then
explicitly retracted it as misreading Bitwig's levels. They confirmed qualitative
channel separation and reported one headphone side carrying tone with the other
silent at Output Jack. The earlier bleed/noise/Mic Input pickup interpretations
...
| P2 | Same, USB Playback L then R; Line Input disconnected | PRELIMINARY: no visible response reported during requested USB Playback with ZERO / Monitor Flag OFF. Bitwig meter moving; Tape pair displayed as "front left and right". After probe exit, operator explicitly reports no headphone playback under requested flag OFF conditions. Exit alone did not restore playback; fresh-boot native baseline and flag ON comparison pending. L/R isolation and disconnected Line Input NOT TESTED. No routing-edge conclusion. |
