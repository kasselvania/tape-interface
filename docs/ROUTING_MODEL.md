...
| P2 | Same, USB Playback L then R; Line Input disconnected | PRELIMINARY: no visible response reported during requested USB Playback with ZERO / Monitor Flag OFF. Follow-up confirms Bitwig meter moving and Tape pair displayed as "front left and right". Individual device meter readbacks, explicit headphone result and native playback comparison pending. L/R isolation and disconnected Line Input NOT TESTED. No routing-edge conclusion. |
